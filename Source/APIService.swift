...
//    //let authenticationAPI: AuthenticationAPIServicing
//    
...
//    }
//    
//    override func request<T>(path: String, method: Alamofire.Method = .GET, parameters: [String: AnyObject]? = nil, encoding: ParameterEncoding = .URL, var headers: [String: String] = [:], authHandler: AuthHandler? = nil, errorHandler: ErrorHandler?  = APIService.errorHandler, onBackground: Bool = false, action: (AnyObject -> (SignalProducer<T, NSError>)) ) -> SignalProducer<T, NSError> {
//        headers = authorizationHeaders(headers)
//        
//        return super.request(path, method: method, parameters: parameters, encoding: encoding, headers: headers, authHandler: (authHandler == nil) ? self.authHandler : authHandler, errorHandler: errorHandler, action: action)
//    }
//}
